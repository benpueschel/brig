use brig_ast::{AstNode, BinOp, Expr, ReturnStmt, LetDecl};
...
impl crate::Ir {
    pub fn traverse_expr_stmt(&mut self, expr: Expr, scope: Scope) -> Res {
        match expr {
            brig_ast::Expr::Call(call) => self.traverse_call_expression(call, scope),
            brig_ast::Expr::Block(block) => Ok(self.traverse_block(block, scope)?.1),
